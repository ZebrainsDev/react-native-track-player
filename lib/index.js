...
function validateTrack(track) {
  const validTrack = {
    ...
    artwork: resolveUrl(track.artwork)
  };
  return validTrack;
}
...
function add(tracks, insertBeforeId) {
  ...
  const validTracks = tracks.map(validateTrack);

  return TrackPlayer.add(validTracks, insertBeforeId);
}

function initQueue(tracks, head) {
  if (!Array.isArray(tracks)) {
    tracks = [tracks];
  }
  ...
  return TrackPlayer.initQueue(validTracks, head);
}
